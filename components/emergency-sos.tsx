...
export default function EmergencySOS() {
  ...
  return (
    <>
      ...
      {/* SOS Modal */}
      {open && (
        <div className="fixed inset-0 z-[10000] flex items-end sm:items-center justify-center p-3 bg-black/70 backdrop-blur-sm">
          <div className="bg-gray-950 border border-red-800 rounded-2xl w-full max-w-md shadow-2xl flex flex-col"
            style={{ maxHeight: "calc(100vh - 24px)" }}>

            {/* Header — sticky */}
            <div className="bg-red-700 rounded-t-2xl px-5 py-3 flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-2">
                <AlertTriangle size={20} className="text-white" />
                <h2 className="text-white font-bold text-base">Emergency Help</h2>
              </div>
              <button
            ...
            {/* Sticky close button at the bottom — always visible */}
            <div className="flex-shrink-0 px-4 py-3 border-t border-gray-800 bg-gray-950 rounded-b-2xl">
              <button
                onClick={() => setOpen(false)}
                className="w-full py-3 rounded-xl bg-gray-800 hover:bg-gray-700 text-white font-semibold text-sm flex items-center justify-center gap-2 transition"
              >
                <X size={16} /> Close Emergency Panel
              </button>
            </div>
